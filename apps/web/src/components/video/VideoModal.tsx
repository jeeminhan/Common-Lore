...
const VideoModal: React.FC<VideoModalProps> = ({ isOpen, onClose, videoUrl }) => {
    ...
    return (
        ...
        >
            <div className={`bg-navy-800 rounded-2xl shadow-2xl flex flex-col overflow-hidden relative border border-white/10 transition-all duration-500 ${
                isMinimized ? "w-full h-full border-white/20" : "w-full max-w-4xl aspect-video"
            }`}>
                ...
                {!isMinimized && (
                    <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
                        <button 
                            ...
                            className="pointer-events-auto mt-60 bg-purple text-white px-8 py-4 rounded-full font-black text-lg shadow-xl hover:scale-110 active:scale-95 transition-all flex items-center gap-3 animate-bounce"
                        >
